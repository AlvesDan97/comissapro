  ...
  asyncHandler(async (req, res) => {
    ...
    if (asaas.enabled()) {
      try {
        ...
        if (!created?.id) {
          return res.status(502).json({ error: 'O Asaas não criou a assinatura. Tente de novo.' });
        }
        if (user.asaas_subscription_id && user.asaas_subscription_id !== created.id) {
          await asaas.cancelSubscription(user.asaas_subscription_id).catch(() => {});
        }
        asaasSub = created.id;
        ...
        return res.status(502).json({
          error: e.message || 'Não foi possível falar com o Asaas. Tente de novo em instantes.',
        });
      }
    }
  ...
  asyncHandler(async (req, res) => {
    ...
    if (asaas.enabled() && user.asaas_subscription_id) {
      ...
      try {
        ...
        return res.status(502).json({
          error: e.message || 'Não foi possível atualizar os assentos no Asaas.',
        });
      }
    }
  ...
  asyncHandler(async (req, res) => {
    ...
    if (user.asaas_subscription_id) {
      try {
        ...
        return res.status(502).json({
          error: e.message || 'Não foi possível cancelar no Asaas. Tente de novo.',
        });
      }
    }
